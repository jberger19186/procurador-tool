...
**Veredicto:** **apto para Beta controlada.** **XSS-1 y NET-1 ya fueron corregidos y verificados el mismo día** (ver estados abajo). El resto son mejoras de robustez sin urgencia (DEP-1, DEP-2 y AUTH-1 ya corregidos el 2026-07-13; BIZ-1/UPL-1 opcionales).
...
| **AUTH-1** | Baja/Info | 2 (sesión) | ✅ **Corregido (2026-07-13)** — device-binding *soft* server-side: `users.machine_id` se fija al login y `execution/start` lo verifica (403 `DEVICE_MISMATCH` si difiere; bind-on-null para sesiones legadas). Sin release (el cliente ya mandaba `machineId`). Verificado en staging + prod |
...
> **Instrucciones concretas para cerrarlos:** `docs/internal/runbook-mejoras-menores-seguridad.md` — con evaluación honesta de esfuerzo/riesgo por ítem. Resumen: **DEP-2** (nodemailer) es el único apto para "ahora, esfuerzo medio" (solo backend + test de envío real); **DEP-1** (Electron) es un proyecto dedicado con release; **AUTH-1** conviene bundlearlo con ese release.

- **DEP-1 (Media) — Electron 28.3.3 (EOL) → ✅ CORREGIDO (2026-07-13):** actualizado a 43.1.0. Se auditaron los breaking changes documentados v29-v43 contra el código real: la app ya seguía patrones modernos (contextIsolation/sandbox explícitos, contextBridge con wrappers, setWindowOpenHandler, sin remote/protocolos deprecados, sin módulos nativos) → cero cambios de código necesarios. Bloqueante resuelto en el camino: `@electron/get` (dependencia de `electron`) requiere Node ≥22.12.0 para el download lazy del binario (cambio de Electron v42) — se actualizó Node local de 22.11.0 a 22.23.1 LTS. Verificado con `npm start` + `.exe` empaquetado real (`build:dir`), afterPack/rcedit intacto, todos los módulos de seguridad inicializando OK. `npm audit` de electron-app también bajó de 26 a 3 vulnerabilidades (moderadas, requieren downgrade breaking de exceljs — diferido).
- **DEP-2 (Baja) — nodemailer <9 → ✅ CORREGIDO (2026-07-13):** actualizado a 9.0.3 (`npm audit` backend en 0 vulnerabilidades). API sin cambios (`createTransport`/`sendMail`). Desplegado con backup pre-deploy + `npm ci` en prod + boot limpio + smoke 8/8 + **envío de email real verificado** (id de mensaje devuelto por el SMTP de Brevo). Staging comparte el mismo `node_modules` (symlink), también actualizado.
- **AUTH-1 (Baja/Info) — JWT no atado a dispositivo → ✅ CORREGIDO (2026-07-13):** el token de sesión (`{id, role}`, 1h) era un bearer estándar sin binding al `machineId` (el `sessionKey` con `machineId` nunca se verificaba). Un token robado funcionaba desde otro dispositivo hasta 1h. **Fix (binding *soft*, server-side, sin release):** el `machineId` se guarda en `users.machine_id` al login (`routes/auth.js` `/login`) — **no viaja en el token**, así que un token robado no revela el dispositivo atado. `POST /license/execution/start` (`routes/license.js`) verifica que el `machineId` del request coincida: NULL → vincula al primer uso (bind-on-null, migra sesiones legadas sin fricción); coincide → permite; difiere → **403 `DEVICE_MISMATCH`**. Política *soft*: cada login re-vincula (cambiar de equipo es transparente). El cliente Electron ya enviaba `machineId` → **no hizo falta release**. Verificado E2E en staging (user 215) y prod (admin id 6, no disruptivo): match→200, distinto→403, NULL→bind. Solo backend, desplegado a prod. Detalle en `runbook-mejoras-menores-seguridad.md`.
...
- El resto (BIZ-1, UPL-1) son mejoras de robustez sin urgencia (DEP-2 y AUTH-1 ya corregidos).
