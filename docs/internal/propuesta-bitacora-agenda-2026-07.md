...
> | **P-F3.4-a** | Export `.ics` — nunca tuvo diseño propio, es serialización de fechas en el módulo de los 3 bugs de timezone de P-F3.0-a. | 📄 Plan listo, Bloque B (`plan-f3-4-semana-e-ics-2026-08.md`) — sin código escrito. |
> | **P-AYUDA-1** | **Bitácora no tiene ninguna entrada en la Ayuda/FAQ de la app ni del portal, ni en el system prompt del asistente de IA** — pese a estar en producción desde F1.1 (2026-08-14). Detectado 2026-08-26 a pedido del operador, al planificar la misma tarea para Markdown. | Sesión dedicada (o junto con M5 de Markdown, ver `plan-modulo-markdown-anonimizacion-2026-08-26.md` §M5): sumar categoría `bitacora` a `FAQ_ITEMS`/`FAQ_CATS` (Electron, `renderer.js`) y `AYUDA_FAQ_ITEMS`/`AYUDA_FAQ_CATS` (portal, `app.js`), más un bullet en `aiSupportPrompt.js`. |
>
...
| **P-F1.3-a** | **Vista "Semana" del toggle no se construyó.** §5.1 mencionaba un toggle [Mes][Semana][Lista] pero sin mockup propio para Semana (a diferencia de Mes y Lista, que sí tenían diseño concreto) — se interpretó como redundante con Mes+Lista para el volumen de datos esperado (una agenda de 1 abogado, no un estudio grande) y se dejó afuera para no inflar el sub-bloque ya marcado como "el más grande del plan". | F1.3 (2026-08-14) | 📄 **Plan de implementación listo (2026-08-16):** `docs/internal/plan-f3-4-semana-e-ics-2026-08.md`, Bloque A (Sonnet, esfuerzo bajo) — puntos de enganche exactos, qué reusar y la única trampa real (`bitacoraApplyViewToggle()` necesita su propio `display` para la vista nueva). Sin código escrito todavía. | 🟡 Abierto (plan listo) |
| **P-F3.4-a** | **Export `.ics` de la agenda — no tenía plan de implementación propio**, solo mencionado como parte de F3.4 en §11 sin diseño. Es una feature de **serialización de fechas**, exactamente el área que produjo los 3 bugs reales de P-F3.0-a. | F3.4 (nunca implementado) | 📄 **Plan de implementación listo (2026-08-16):** `docs/internal/plan-f3-4-semana-e-ics-2026-08.md`, Bloque B (Sonnet, esfuerzo medio) — va como tercer `formato` del endpoint `/bitacora/export` que ya existe (hereda el gate con ventana de gracia de 90 días), con las 6 trampas específicas del modelo de datos documentadas en §B.2 (`due_at` es un `timestamptz` a mediodía local, `all_day`, entradas sin fecha, escapado/plegado RFC 5545, `UID` estable, `repeat_rule`→`RRULE`). Sin código escrito todavía. | 🟡 Abierto (plan listo) |
...
| **P-AYUDA-1** | **Bitácora no tiene ninguna entrada en las 3 superficies de ayuda del producto.** `electron-app/renderer.js` (`FAQ_ITEMS`/`FAQ_CATS`) y `backend-server/public/usuarios/app.js` (`AYUDA_FAQ_ITEMS`/`AYUDA_FAQ_CATS`) tienen las mismas 7 categorías idénticas (procuración/informe/monitor/extensión/cuenta/errores/privacidad) — sin ninguna mención a Bitácora, pese a que el módulo lleva en producción desde 2026-08-14 (F1.1) y hoy incluye agenda, expedientes seguidos, sugerencias automáticas del Monitor (F3.3), vista Semana y export `.ics` (F3.4). Tampoco aparece en `backend-server/utils/aiSupportPrompt.js` (el system prompt único del asistente de IA, compartido por el chat de Electron y el del portal) — un usuario que le pregunte al bot "¿qué es Bitácora?" hoy recibe una respuesta genérica o inventada, contra la propia REGLA DE ORO del prompt de "nunca inventar funciones". Detectado 2026-08-26, a pedido del operador, al planificar la misma tarea para el módulo Markdown (ver `plan-modulo-markdown-anonimizacion-2026-08-26.md`, sección M5). | Descubierto planificando Markdown (2026-08-26), aplica a todo F1-F3.4 | Sesión dedicada, o combinada con la tarea equivalente de Markdown en **M5** (mismos 3 archivos, para no hacer 2 pasadas separadas): agregar categoría `bitacora` con ~6-8 preguntas (agenda, expedientes seguidos, sugerencias del Monitor, export, backup/restore) a las 2 FAQ + un bullet en el system prompt. | 🟡 Abierto (gap real de un módulo ya en producción, no un bug) |
