...
|---|---|---|---|
| **D1** | **Clave de deduplicación del expediente** — cómo se evita que `FCR 018745/2017` (forma del PJN) y `FCR 18745/2017` (forma que tipea el usuario) creen dos fichas del mismo caso. *(Hallazgo N1)* | ✅ **Confirmado.** Se agrega columna **`expediente_key`** con el número normalizado, y el `UNIQUE` va sobre ella en vez de sobre el texto visible. La normalización **reusa `tokenizar()`** de `electron-app/informe/buscarPdfExpediente.js` (código ya probado, nacido del bug `debb503`). Se conserva `expediente` con el texto original, que es lo que el usuario reconoce. **Precisado el 2026-08-13** (auditoría externa B1/B2/C3): la clave es **`UNIQUE (user_id, expediente_key)` — `jurisdiccion` NO forma parte** (es redundante, porque la sigla ya viaja dentro de la clave, y peligrosa, porque llega como texto libre distinto según el origen); y la normalización vive **canónica en `backend-server/utils/expedienteKey.js`** + copia en Electron + **fixture de casos compartido** para que no deriven. Ver §7 de la propuesta. | Es una columna y una constraint: está en la migración o no está. Descubrirlo después obliga a migrar fichas reales de usuarios y deduplicar a mano las que ya se partieron. |
