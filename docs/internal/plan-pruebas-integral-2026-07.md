...
| A6.2 | Dos checkouts seguidos (single-active) | 1 solo preapproval vivo; el viejo cancelado | ⏭️ Intentado, no completado — usuario 239 ya tenía un preapproval real autorizado (`33d45ac93b1c46659e248c70d188b057`); se navegó a Facturación y se clickeó "Cambiar método" para iniciar un 2do checkout sin cancelar el primero, pero **Chrome volvió a quedar inestable a nivel de toda la sesión** (ni una pestaña nueva pudo navegar) antes de completar el checkout. `checkout_initiated_at` quedó en `NULL` (el click no llegó a disparar la request). Usuario 239 no quedó afectado (preapproval original intacto) |
