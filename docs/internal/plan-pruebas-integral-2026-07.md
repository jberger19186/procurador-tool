...
| A6.2 | Dos checkouts seguidos (single-active) | 1 solo preapproval vivo; el viejo cancelado | ⏭️ **Reintentado 2 veces, bloqueado ambas por el freeze del Hallazgo #2** — usuario 239 ya tenía un preapproval real autorizado (`33d45ac93b1c46659e248c70d188b057`); clickear "Cambiar método" (misma función `initCheckout()` que otros checkouts exitosos previos) cuelga el renderer de forma reproducible cuando el usuario ya tiene `payment_provider` seteado — ver Hallazgo #2 (evidencia ampliada). `checkout_initiated_at` nunca se actualiza. Usuario 239 no quedó afectado (preapproval original intacto) |
...
| U9.3 | Pagar reactivación | Cuenta activa con plan nuevo | ⏭️ Bloqueado consistentemente por el mismo freeze del Hallazgo #2 en 8 intentos distintos. Nota: este caso (`suspended_plan_expired`) también involucra un usuario que ya tenía `payment_provider` configurado antes de vencer, consistente con la hipótesis ampliada del Hallazgo #2 (el freeze ocurre en `initCheckout()` cuando el usuario ya tuvo un método de pago). Usuario 239 siempre restaurado a `active` limpio tras cada intento, sin transacciones a medias |
...
| 2 | ⚠️ Ahora con más evidencia de ser real (no solo automatización) | Operativo/Bug candidato | U9.3, A6.2 | Al clickear **"Seleccionar este plan"** (modal de reactivación, `suspended_plan_expired`) el tab de Chrome queda **completamente sin responder** (ni screenshot ni JS eval — `document.title` timeoutea a los 45s) hasta abrir un tab nuevo. Reproducido **8 de 8 veces** en 4 tabs distintos, 4 sesiones separadas del 2026-07-03. **Nueva evidencia (misma tarde, sesión de corrección):** el mismo freeze exacto (confirmado con `document.title` vía JS, no solo screenshot) ocurrió también al clickear **"Cambiar método"** en Facturación (usuario 239, que YA tiene `payment_provider='mercadopago'` seteado) — botón que llama la **misma función** `initCheckout()` (sin argumento) que "Configurar método de pago", la cual **sí funcionó sin problema 3 veces antes en la misma sesión** para un usuario SIN método de pago. Es decir: `initCheckout()` funciona bien cuando `payment_provider` es `NULL`, pero se cuelga de forma reproducible cuando el usuario **ya tiene un método configurado** (branch que en el backend usa `createUpdatePreapproval` en vez del checkout plan-based, según lo documentado en CLAUDE.md). Confirmado por DB en ambos casos que `checkout_initiated_at` nunca se actualiza — el freeze ocurre en el cliente **antes** de que la request llegue al servidor. Revisado el código de `initCheckout()` en `usuarios/app.js`: no hay ningún `confirm()`/diálogo nativo que explique un bloqueo esperable: solo un `await apiFetch(...)` estándar, por lo que un freeze total del renderer (no solo de una promesa pendiente) es inesperado desde una lectura estática del código | Dado el patrón repetido en 2 botones distintos que comparten la misma función bajo la misma condición (`payment_provider` ya seteado), **priorizar la investigación de `initCheckout()`/`apiFetch()`** en `usuarios/app.js` específicamente para el camino "cambiar método existente" — revisar si hay algo en el manejo de la respuesta de `/usuarios/api/checkout/init` (rama `createUpdatePreapproval`) que pueda colgar el hilo del renderer (ej. un `while` esperando una condición que nunca se cumple, o un recurso que nunca libera). Sigue pendiente descartar formalmente que sea un artefacto de automatización con un click humano real, pero la repetición en 2 contextos distintos hace más probable que sea un bug de producto |
