...
| A6.2 | Dos checkouts seguidos (single-active) | 1 solo preapproval vivo; el viejo cancelado | ⏭️ **Bloqueado por el bug confirmado del Hallazgo #2** — "Cambiar método" (usuario 239, ya con preapproval real autorizado) dispara `createUpdatePreapproval()`, cuyo `reason` de 66 caracteres excede el límite de 40 de MercadoPago; MP rechaza el preapproval y el frontend muestra el error crudo con `alert()` nativo, bloqueando la página. No se puede completar A6.2 hasta corregir el `reason` en `subscriptionService.js`. Usuario 239 no quedó afectado (preapproval original intacto, sin transacciones a medias) |
