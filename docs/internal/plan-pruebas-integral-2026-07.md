...
| U9.3 | Pagar reactivación | Cuenta activa con plan nuevo | ⏭️ **Bloqueado por el bug confirmado del Hallazgo #2** — "Seleccionar este plan" en la reactivación dispara `createReactivationPreapproval()`/`createUpdatePreapproval()`, cuyo `reason` excede el límite de 40 caracteres de MercadoPago (sufijo `"(reactivación · N días ya abonados)"`); MP rechaza el preapproval y el frontend cuelga con un `alert()` nativo mostrando el error crudo. **No es un problema de la app de pruebas ni de la automatización** — cualquier usuario real que intente reactivar su cuenta desde el portal se va a topar con este mismo bloqueo. No se puede completar U9.3 hasta corregir el `reason`. Usuario 239 siempre restaurado a `active` limpio tras cada intento, sin transacciones a medias |
