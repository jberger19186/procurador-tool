...
| 3 | **Medio-Alto** | **Bug (funcional, confirmado)** | U12.8 | El pre-check de límite por submódulo **no bloquea la procuración** cuando el usuario ya alcanzó su límite. Reproducido con la app Electron real (usuario 243, plan COMBO_PROMO): forcé `proc_usage=50` (=`proc_executions_limit`, confirmado `remaining:0` vía `GET /client/account`) y clickeé "Procurar" (dispara el handler `run-process-custom-date` en `main.js`, que en el código **sí** llama a `checkSubsystemLimit('proc', ...)` en la línea ~1252 y debería retornar `blocked:true` con `remaining<=0`) — pero la app **abrió Chrome igual, inició sesión real en el PJN y completó una procuración de 3 expedientes reales en 42s**, sin ningún toast ni bloqueo. El servidor sí protegió la integridad de los datos (`log-execution` rechazó silenciosamente el conteo — `proc_usage` se mantuvo en 50, no subió a 51+), pero el usuario **no vio ningún aviso**: la ejecución "parece" exitosa (visor con 3/3, sin errores) aunque no cuenta para nada, generando una discrepancia confusa entre lo que el usuario ve y lo que el sistema realmente registra. Impacto: consumo innecesario de tiempo/recursos (automatización Puppeteer completa contra el PJN real) y confusión del usuario sobre por qué "no le suma" a sus contadores | Investigar por qué `checkSubsystemLimit()` no bloqueó pese a que el código fuente muestra la lógica correcta — posibles causas: (a) `sessionInfo`/`getAccount()` devolviendo datos cacheados/stale en el momento del click, (b) alguna condición previa en el handler que hace early-return antes de llegar a la línea del check, o (c) el click de "Procurar" en el nav superior invoca un handler distinto al que tiene el código revisado. Agregar un test automatizado que cubra este camino específico (proc_usage=límite exacto → click Procurar → debe bloquear sin abrir Chrome) para evitar regresiones |
