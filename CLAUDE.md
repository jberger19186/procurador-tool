...
| ~~**D3**~~ | ~~**`npm audit fix` (sin --force)**~~ | ✅ Hecho (2026-07-12) en backend: 19→1 vulnerabilidades (deployado a prod). En el camino se detectó y corrigió un drift real (`multer` faltaba en `package.json`, solo vivía en el `node_modules` del server). **D3-electron revisado (2026-07-14) y reclasificado junto a D4:** las 3 vulnerabilidades (`uuid` moderate, vía `exceljs`/`node-notifier`) solo tienen fix con `--force` (downgrade de `exceljs` a 3.4.0, breaking — motor del Excel de informes). No explotable en nuestro uso. Diferir a la misma ventana que D4 | — |
| **D4** | **`npm audit fix --force` controlado** | Deps con breaking changes (mercadopago/uuid, axios, undici en backend; `uuid`→downgrade de `exceljs` en electron-app, ver D3-electron arriba). Probar flujo de pagos completo en staging (backend) y el informe Excel (electron) después de actualizar. Diferido a justo antes de B3 (lib de MP fresca al activar el cobro real) | Baja |
