...
| **D4** | **`npm audit fix --force` controlado (solo backend)** | Deps con breaking changes en backend (mercadopago/uuid, axios, undici). D3-electron ya se resolvió sin breaking changes (ver arriba) — D4 queda acotado al backend. Probar flujo de pagos completo en staging después de actualizar. Diferido a justo antes de B3 (lib de MP fresca al activar el cobro real) | Baja |
