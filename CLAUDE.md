...
> 6. ⭐ **`docs/internal/plan-Q6-verificacion-firmas-2026-07-28.md`** — cierra los **9 fail-opens** del sistema de verificación de firmas de scripts. **✅ Fase 1 (backend, C1/F9) EJECUTADA Y EN PRODUCCIÓN (2026-07-29, commit `8d01090`)** — el backend ya no sirve scripts sin firma (500 en vez de degradar), 13/13 verificados en staging y prod. **⏸️ Ventana de espera activa: 24-48 h monitoreando `/var/log/procurador/pm2-error.log` por `[SEGURIDAD] No se pudo firmar`** (0 coincidencias al desplegar) antes de arrancar la Fase 2. **Las 4 decisiones del operador ya estaban tomadas:** fail-closed · C7 en release propio · sin kill switch · mensaje único al usuario con detalle solo en el log. **Cero cambios en `src/security/`** (zona protegida). **Para continuar (tras confirmar el log limpio):** sesión nueva, Sonnet ALTO → «Ejecutá la Fase 2 del plan `docs/internal/plan-Q6-verificacion-firmas-2026-07-28.md`» (endurece el cliente, release v2.7.45) → luego Fase 3 (etapa 3 real, release v2.7.46). **⚠️ Nota para la sesión de la Fase 2:** el release v2.7.45 **también carga 3 fixes ya commiteados y verificados** (commits `c681ae8`, `822bf0d` y `4c648fc`: links a PDF del informe por lote · notificación de proceso completado · expediente marcado "Exitoso" sin haber generado informe) — no son parte de Q6, están en `informe/` y en el call site de notificaciones de `authManager.js`, sin interacción con la ruta de verificación de firmas. Basta con mencionarlos en el release; la §5.1 de no-regresión ya los ejercita.
>
