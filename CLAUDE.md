...
> Última sesión: 2026-06-27 (**Fase 2 COMPLETA** de vigencia de planes por fecha: núcleo (cron de retiro respeta el período + **pausa MP**, fin de período → `suspended_plan_expired` con gracia 7d, aviso 7 días, commit `30c59d6`) + **Change 3 reactivación real** (el vencido elige plan activo y paga por checkout de MP; `change-plan` ya no reactiva gratis, commit `9939474`); validado en staging, dormido en prod hasta setear `plan_expiry_date`. **Fix D1** `GRANT` a `procurador_user` (arregla "permission denied for sequence" al aplicar beneficio). Fixes previos: `invoices.updated_at`, resaltado de fila persistente.)
...
- ✅ **Sesión 2026-06-27 — Fase 2 (núcleo) vigencia de planes + fixes facturación/UX** :
  ...
  - **Resguardos:** `.7z` `202606_27062026_ProcuradorTool.7z` (+ `…_fase2.7z` al cierre) + tag `pre-fase2-vigencia-2026-06-27`.
