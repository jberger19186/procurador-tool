...
| ~~**D3**~~ | ~~**`npm audit fix` (sin --force)**~~ | ✅ Hecho (2026-07-12): backend 19→1 vulnerabilidades (deployado a prod). En el camino se detectó y corrigió un drift real (`multer` faltaba en `package.json`, solo vivía en el `node_modules` del server). electron-app pendiente de correr (D3-electron no ejecutado aún) | — |
| **D4** | **`npm audit fix --force` controlado** | Deps con breaking changes (mercadopago/uuid, axios, undici). Probar flujo de pagos completo en staging después de actualizar. Diferido a justo antes de B3 (lib de MP fresca al activar el cobro real) | Baja |
