...
> 5. ⭐ **`docs/internal/plan-correcciones-E1-E6-2026-07-27.md`** — **el trabajo más accionable, EN EJECUCIÓN.** Plan de los ~35 hallazgos de E1-E6, organizado por vector de despliegue: ~~**Bloque A** backend+dashboard~~ ✅ **HECHO (2026-07-28, commit `03e294d`) — P-1 cerrado** · **Bloque B** base de datos (regenerar `schema.sql` + índices) · **Bloque C** motor Puppeteer (reencrypt, sin release de Electron, el de mayor valor) · **Bloque D** release de Electron (incluye el fix de P-2) · **Bloque E** extensión (fix del logo con SSO decidido). **Q1/Q2/Q3/Q5 ya resueltas** (logo con SSO automático · se descartó el ítem de jurisdicción · `schema.sql` se automatiza en el backup diario · el downgrade programado se cancela al suspender); quedan **Q4/Q6/Q7**. **Próximo recomendado: Bloque E** (es el único que depende de un tercero — el ciclo de revisión de Google — así que conviene arrancarlo para que corra en paralelo), luego **B**. **Para retomar:** sesión nueva → «Ejecutá el Bloque [letra] del plan `docs/internal/plan-correcciones-E1-E6-2026-07-27.md`».
>
