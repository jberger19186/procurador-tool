...
> 5. ⭐ **NUEVO (2026-07-27) — `docs/internal/plan-correcciones-E1-E6-2026-07-27.md`** — **el trabajo más accionable que hay hoy.** Plan de ejecución de los ~35 hallazgos de E1-E6, organizado por vector de despliegue: **Bloque A** backend+dashboard (sin release) · **Bloque B** base de datos (regenerar `schema.sql` + índices) · **Bloque C** motor Puppeteer (reencrypt, sin release de Electron, el de mayor valor) · **Bloque D** release de Electron (incluye el fix de P-2) · **Bloque E** extensión (recomendado arrancar en paralelo — incluye el diseño del fix del logo, ver más abajo). 7 preguntas abiertas con recomendación en §7 (la más relevante: **Q1**, SSO automático vs. login manual para el logo de la extensión). **Para arrancar:** sesión nueva → «Ejecutá el Bloque A del plan `docs/internal/plan-correcciones-E1-E6-2026-07-27.md`».
>
> **🧩 PENDIENTES ACUMULADOS PARA LA PRÓXIMA ACTUALIZACIÓN DE LA EXTENSIÓN (2026-07-25) — 3 puntos, ninguno corregido aún, solo documentados:**
> 1. **Logo sin acción de click** — ⭐ **diseño completo listo en `plan-correcciones-E1-E6-2026-07-27.md` §6 (Bloque E.1), agregado 2026-07-27 a pedido del operador.** En `extension-app/popup.html:412-419`, el bloque `.header` (logo + "Procurador TOOL") es **compartido** entre `#view-login` y `#view-main` (vive fuera de ambos `<div>`) → un solo listener alcanza, pero debe leer el estado **en el momento del click** (no al cargar la popup). Comportamiento pedido: **sin sesión** → abre `https://procuradortool.com`; **con sesión** → abre el dashboard del usuario. Verificado que el **SSO automático al dashboard es técnicamente viable** (la extensión guarda un JWT, el portal soporta `#sso=`, mismo `JWT_SECRET`, ningún endpoint valida el claim `client`) — **pendiente de decisión: Q1** (SSO automático, recomendado, vs. login manual) y a qué sección del portal cae (default propuesto: home del panel). (Hallazgo original de R9.1/R9.2, probado a mano por el operador contra la extensión real v1.3.6.)
> 2. **Carga de jurisdicción a revisar en el flujo de Notificaciones** — `extension-app/cs-notif.js` (función `fillFields`, líneas ~88-123) rellena el combobox de jurisdicción con `reactSet()` + búsqueda de opción por `innerText.includes(jurisdiccion)` o por sigla (`SIGLA_FROM_LABEL`) + `Enter`. El operador marcó que el llenado de jurisdicción en este flujo específico necesita control/revisión (no especificado si selecciona mal, tarda, o falla en casos borde) — a diferencia de Escritos 2 y DEOX que comparten un patrón similar y no fueron señalados. Investigar contra el DOM real de `notif.pjn.gov.ar/nueva` antes de tocar el código. (Hallazgo de R9.1/R9.2.)
> 3. **`checkExtensionVersion()` huérfano en `background.js`** — sigue llamando a `GET /api/extension/version`, endpoint **eliminado** en RI-4 (2026-07-22, ver sesión "Sesión 2026-07-22"). Falla en silencio, no rompe nada — limpieza de código muerto, sin urgencia. (Detectado en la revisión de bugs de la extensión, sesión 2026-07-25 cont. 3.) **Por qué agruparlo con los otros 2 en vez de corregirlo ya:** cada bump de versión de la extensión implica subirla al dashboard y esperar el ciclo de revisión de Google (mismo día en el mejor caso, pero no garantizado) — conviene juntar los 3 fixes en un solo bump para no gastar un ciclo de revisión extra por cada uno.
>
