...
>
> **🧩 Extensión Chrome — los 2 pendientes de código quedaron cerrados en v1.3.7 (aprobada 2026-07-28):**
> 1. ~~Logo con acción de click, según estado de sesión~~ — ✅ implementado en el Bloque E (SSO automático, decisión Q1). Único remanente: **confirmar visualmente el click en Chrome real** (sin sesión → `procuradortool.com`; con sesión → panel logueado) — la lógica ya se verificó por simulación, falta la confirmación a ojo.
...
> FCR 18745/2018
> ```
> (mismo contenido usado en la sesión de referencia — no inventar otros expedientes salvo que el operador pida otros explícitamente).
>
...
>
> **3. Los 5 flujos:**
> 1. **Procuración común** — corregir "Fecha límite" a la fecha de HOY (no confiar en el valor guardado de una sesión anterior) → botón "Procurar".
> 2. **Procuración por lote** — sidebar "Por lote" → seleccionar `batch.txt` → "Procesar".
> 3. **Informe individual** — tab "Informe" → pestaña "Individual" → expediente `FCR 18745/2017` (si el campo trae un valor recordado de otra sesión, seleccionar todo y retipear — a veces el valor pre-cargado no dispara la validación interna y tira "Ingresá el número de expediente") → "Ejecutar".
> 4. **Informe por lote** — tab "Informe" → pestaña "Batch (.txt)" → seleccionar el mismo `batch.txt` → "Ejecutar".
> 5. **Monitor — Buscar Novedades** — tab "Monitor" → si ya hay partes cargadas, tildarlas y clickear "Buscar Novedades". **Si no hay ninguna cargada**, agregar 2 partes nuevas jurisdicción **FCR**, nombre **"DON COCHO"** y **"LA TOSTADORA MODERNA"** (botón "+ Agregar parte" → completar jurisdicción/nombre → guardar) y recién ahí correr "Buscar Novedades" sobre esas dos.
>
> **Qué mirar en cada resultado:** el visor HTML se abre solo en Chrome al terminar cada flujo (no se puede clickear ahí — leer con `screenshot`, o verificar por archivo con `Read`/`Bash` en `%APPDATA%\procurador-electron\usuarios\<CUIT>\descargas\`). Confirmar "Exitosos" = total, "Fallidos" = 0. Para el informe, además: que el botón "Abrir PDF" esté activo (no "N/A") — si algún expediente da error o el PDF no linkea, es una regresión a investigar antes de seguir, no un detalle a ignorar.
>
