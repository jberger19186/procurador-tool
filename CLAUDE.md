...
| ~~**SEC-1**~~ | **Auditoría de seguridad** | ✅ Ejecutada (2026-07-13) | 📄 **Informe:** `docs/internal/informe-seguridad-sec1-2026-07-13.md`. 7 bloques (~30 pruebas white+black-box contra staging). **Postura sólida** — sin críticos, sin SQLi/IDOR/fuga de credenciales. **2 hallazgos accionables nuevos:** 🔴 **XSS-1 (Alto)** — stored XSS en el dashboard admin (título de ticket, `nombre_parte`, emails renderizados sin `escHtml` → un usuario común roba el token admin; corregir antes de Beta); 🟠 **NET-1 (Medio)** — Express en `*:3443`/`*:3444` accesible desde internet sin firewall (saltea Nginx + basic-auth de staging; activar `ufw` o bindear a localhost). Menores: DEP-1 (Electron 28 EOL), DEP-2 (nodemailer <9), AUTH-1 (JWT no atado a dispositivo). La externa profesional sigue recomendada antes del público |
