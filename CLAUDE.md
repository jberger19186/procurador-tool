...
## 🔄 Estado actual
> Versión app Electron: **2.7.43** — publicada en GitHub Releases (auto-updater activo)
> Versión extensión Chrome: **1.3.6 activa en el Chrome Web Store** · **1.3.7 lista en el repo** (commit `1f9d1c4`, ZIP generado) — ⏳ pendiente de que el operador la suba al dashboard de Chrome Web Store.
...
> 5. ⭐ **`docs/internal/plan-correcciones-E1-E6-2026-07-27.md`** — **el trabajo más accionable, EN EJECUCIÓN.** Plan de los ~35 hallazgos de E1-E6, organizado por vector de despliegue: ~~**Bloque A** backend+dashboard~~ ✅ **HECHO (2026-07-28, commit `03e294d`) — P-1 cerrado, sin pendientes** · ~~**Bloque B** base de datos~~ ✅ **HECHO (2026-07-28) — schema.sql regenerado + su regeneración automatizada en el backup diario (Q5) + 4 índices** · ~~**Bloque C (C.1+C.2)** motor Puppeteer~~ ✅ **HECHO Y EN PRODUCCIÓN (2026-07-28) — los 13 fixes de robustez/aislamiento/limpieza** (falta solo la verificación funcional real contra el PJN, que necesita al operador) · **Bloque D** release de Electron (incluye el fix de P-2) · ~~**Bloque E** extensión~~ ✅ **CÓDIGO LISTO (2026-07-28, commit `1f9d1c4`), v1.3.7 — subida al Chrome Web Store, ⏳ en revisión de Google**. **Q1/Q2/Q3/Q4/Q5 ya resueltas**; quedan **Q6/Q7**. **Próximo recomendado: Bloque D** (release de Electron, el único bloque de código que falta). **⚠️ Aprendizaje de esta sesión (aplicado ya en la 2ª mitad sin incidentes):** `reencrypt_scripts.js` tiene el mismo bug de `.env` sin path que `backup-db.js` (Bloque B) — cualquier corrida manual "en staging" de estos scripts debe forzar `node -r dotenv/config <script> dotenv_config_path=.env.staging`, si no termina tocando producción. **Para retomar:** sesión nueva → «Ejecutá el Bloque [letra] del plan `docs/internal/plan-correcciones-E1-E6-2026-07-27.md`».
>
