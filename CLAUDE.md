...
### 🔍 Revisión de código — terreno NO cubierto (nuevo 2026-07-27)

> 📄 **Plan completo: `docs/internal/plan-revision-integral-2026-07-27.md`** — incluye el mapa de
> lo que YA está revisado (§1, para no repetir trabajo), el detalle de cada bloque (§4), la
> recomendación de modelo/esfuerzo (§5) y las reglas de ejecución (§6).
> **Regla de ejecución:** una sesión por bloque, contexto fresco. La pasada de revisión **no
> modifica código** — produce el informe y el operador decide qué se corrige.
> **Para arrancar:** «Ejecutá el Bloque E1 del plan `docs/internal/plan-revision-integral-2026-07-27.md`».

| # | Bloque | Superficie | Modelo/esfuerzo | Prioridad |
|---|---|---|---|---|
| **E1** | **Motor de automatización Puppeteer** | `backend-server/scripts/` (~370 KB, 13 scripts vivos). Empezar por `testM2.js` (101 KB), la librería núcleo que usan todos. **Nunca revisado — es lo que el cliente paga.** Un fix acá **no requiere release de Electron** (editar + `reencrypt_scripts.js` + `pm2 restart`) | Sonnet **ALTO** · 2 sesiones | 🔴 **La más alta** |
| **E2** | **Electron: `main.js` + `preload.js` + `src/`** | `main.js` 126 KB (~60 handlers IPC), `authManager.js` 44 KB, `backendClient`, `scriptExecutor`, `dailyVerification`. **Excluir `src/security/`** (zona protegida: leer sí, corregir no sin decisión explícita) | Sonnet **ALTO** · 2 sesiones | 🟠 Alta |
| **E3** | **Los 11 crons y la máquina de estados** | `server.js:524-875`. **Acotar a lo NO cubierto:** la lógica interna de los de cobranza ya se revisó (B1/B5/B6) → este bloque va por la **interacción** entre ellos (solapamiento, orden, idempotencia del conjunto, estados imposibles, queries sin índice) | Sonnet **ALTO** · 1 sesión | 🟠 Alta |
| **E4** | **Visores / Excel / onboarding** | `generador_visor.js`, `generador_excel.js`, `visorModal_template.html`, el `datosEmbebidos` de los scripts encriptados, `onboarding.js` + `tour.js`. **Arranca con P-2 ya detectado** (XSS potencial) + formula injection en Excel + pasos del tour rotos tras v2.7.33-43 | Sonnet **MEDIO** · 1 sesión | 🟡 Media (barato y con hallazgo concreto) |
| **E5** | **Schema de DB + distribución de scripts** | Las 27 tablas (índices, constraints, tipos, drift de migraciones) + `reencrypt_scripts.js` + los 2 endpoints de scripts. **Arranca con P-1 ya detectado** | Sonnet **MEDIO** · 1 sesión | 🟡 Media |
| **E6** | **Frontends** | `dashboard.js` (313 KB) + `usuarios/app.js` (132 KB). **Acotar por riesgo, NO leer completo** — método: grep dirigido (`innerHTML` con interpolación, `fetch(` contra rutas muertas, token en `localStorage`) y leer solo lo que el grep marque | Sonnet **MEDIO** · 1-2 sesiones | 🔵 Baja |

**Hallazgos ya detectados al armar el plan (NO corregidos — ver §3 del doc):**
- **P-1** 🔵 bajo-medio — `/client/scripts/download/:name` y `/scripts/available` (`routes/client.js:153` y `:238`) no filtran por script ni por plan → cualquier trial vivo puede bajar los 5 scripts de **operación del servidor** que `reencrypt_scripts.js` barre del directorio. **Sin secretos hardcodeados** (verificado: todo sale de `process.env`); expone arquitectura interna, no credenciales.
- **P-2** 🟡 medio (a confirmar) — visores con `innerHTML` (×6) y **cero** escapado en `visorModal_template.html` / `generador_visor.js` / `procesarNovedadesCompleto.js`, con datos del PJN embebidos. Mismo patrón de clase que XSS-1 y D2.

**Lo que este plan NO cubre** (explícito, para no confundirlo con "proyecto 100% revisado"): R9.1/R9.2 · los 3 pendientes de la extensión · auditoría de seguridad **externa** profesional · **pruebas de carga/concurrencia real** (nunca hechas; hueco real de cara a B3) · `electron-app/src/security/`.
