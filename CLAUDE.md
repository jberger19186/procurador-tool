...
## 🔄 Estado actual
> Versión app Electron: **2.7.44** — publicada en GitHub Releases (auto-updater activo)
...
> 6. ⭐ **`docs/internal/plan-Q6-verificacion-firmas-2026-07-28.md`** — cierra los **9 fail-opens** del sistema de verificación de firmas de scripts. **✅ Fase 1 (backend, C1/F9) EN PRODUCCIÓN** (2026-07-29, commit `8d01090`) · **✅ Fase 2 (cliente, C2-C6) EJECUTADA Y VERIFICADA** (2026-07-30, commit `e080809`) — los 5 fail-opens de `authManager.js` cerrados, 19/19 aserciones automatizadas PASS contra el código real con criptografía RSA genuina (no mocks de la lógica bajo prueba). **⏸️ ÚNICO PASO PENDIENTE ANTES DEL RELEASE v2.7.45: correr los 3 flujos reales contra el PJN con el código ya endurecido** (§5.1 punto 2 del plan) — requiere al operador, la verificación automatizada no reemplaza esa confirmación. Tras eso: bump de versión → tag `electron-v2.7.45` → `npm run release` → deploy → verificar en vivo (checklist estándar de este documento). Después de publicar v2.7.45, dejarlo correr unos días antes de la **Fase 3** (etapa 3 real, C7/F6, release v2.7.46 — la más delicada de las tres, toca el camino crítico de cada ejecución). **Cero cambios en `src/security/`** en toda la Fase 2 (zona protegida). **El release v2.7.45 también carga 4 fixes ya commiteados y verificados, ajenos a Q6:** commits `c681ae8`, `822bf0d`, `4c648fc` (links a PDF del informe por lote · notificación de proceso completado · expediente marcado "Exitoso" sin haber generado informe) y `e080809` en su segunda mitad (expediente inexistente ya no consume cupo de `informe_usage`) — la §5.1 de no-regresión ya los ejercita, no hace falta nada extra por ellos.
>
