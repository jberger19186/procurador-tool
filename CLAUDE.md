...
> Versión extensión Chrome: **1.3.6 activa en el Chrome Web Store** · **1.3.7 subida por el operador** (commit `1f9d1c4`) — ⏳ en revisión de Google.
...
> 6. ⭐ **`docs/internal/plan-Q6-verificacion-firmas-2026-07-28.md`** — **el trabajo listo para ejecutar más accionable que queda.** Cierra los **9 fail-opens** del sistema de verificación de firmas de scripts (E2-2 documentaba 3; la investigación encontró 6 más, incluido que **la etapa 3 no verifica el disco sino la RAM**, o sea que esa capa de defensa hoy no defiende nada). Decisión del operador ya tomada: **fail-closed**. **Cero cambios en `src/security/`** (zona protegida) — se cierra desde el llamador vía `isReady()`. **⚠️ Secuencia obligatoria: Fase 1 (backend) → esperar 24-48 h monitoreando el log → Fase 2 (cliente + release v2.7.45).** Ejecutar con **Sonnet ALTO**. Quedan 3 decisiones para el operador en §7 (alcance de C7, kill switch, texto del mensaje de error). **Para arrancar:** sesión nueva → «Ejecutá la Fase 1 del plan `docs/internal/plan-Q6-verificacion-firmas-2026-07-28.md`».
...
> 2. **`checkExtensionVersion()` huérfano en `background.js`** — sigue llamando a `GET /api/extension/version`, endpoint **eliminado** en RI-4 (2026-07-22). Falla en silencio, no rompe nada — limpieza de código muerto, sin urgencia.
>
> ~~Carga de jurisdicción en Notificaciones~~ — **descartado (2026-07-27):** el operador confirmó que ya no es un problema, se elimina de la lista de pendientes.
>
...
### Versión en store: 1.3.6 ✅ aprobada por Google (25/07/2026) · Versión local: 1.3.6
