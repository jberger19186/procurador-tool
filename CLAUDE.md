...
> 2. **`docs/internal/propuesta-bitacora-agenda-2026-07.md`** — propuesta Bitácora v6.1 revisada (5 correcciones acumuladas: Nginx 20M, body-limit con orden de parsers, 3 columnas, UNIQUE con jurisdiccion NOT NULL, protecciones del POST de captura anónimo) + plan de fases §11/§11.1 con modelo/esfuerzo por sub-bloque. **Sigue NO aprobada** — requiere ok del operador antes de implementar.
> 3. **`docs/internal/plan-pruebas-integral-2026-07.md`** — BLOQUE R (37 casos) con spec de automatización (22 AUTO/9 SEMI/6 OP, 4 tandas) + tabla de modelo/esfuerzo por etapa + regla: si el ejecutor quiere cambiar de modelo, **informa y espera confirmación del operador antes de seguir**. **✅ Ejecutado 37/37** — solo quedan R9.1/R9.2 (extensión real, requiere presencia del operador). Es **registro histórico**, no trabajo pendiente.
> 4. ⭐ **NUEVO (2026-07-27) — `docs/internal/plan-revision-integral-2026-07-27.md`** — **el trabajo de revisión más accionable que hay hoy.** Plan de 6 bloques (E1-E6) sobre el terreno que **ninguna** revisión tocó: **E1 motor de automatización Puppeteer** (~370 KB, nunca revisado, es lo que el cliente paga — empezar por `testM2.js`, la librería núcleo) · **E2 Electron `main.js`/`preload.js`/`src/`** · **E3 los 11 crons y la máquina de estados** · **E4 visores/Excel/onboarding** · **E5 schema de DB + superficie de distribución de scripts** · **E6 frontends**. Incluye **§1 mapa de cobertura de lo YA revisado (qué NO re-testear)**, **§3 con 2 hallazgos ya detectados** (P-1 scripts de operación del servidor descargables por cualquier trial · P-2 XSS potencial en los visores), **§5 modelo/esfuerzo por bloque** y **§6 reglas de ejecución**. **Para arrancar:** sesión nueva → «Ejecutá el Bloque E1 del plan `docs/internal/plan-revision-integral-2026-07-27.md`». Mayor retorno con menor gasto: **E1 + E4**.
>
...
> 1. **Logo sin acción de click** — en `extension-app/popup.html:412-419`, el bloque `.header` (logo + "Procurador TOOL") es **compartido** entre `#view-login` y `#view-main` (vive fuera de ambos `<div>`, un solo header para toda la popup) → un solo listener alcanza para los dos estados. Falta: al hacer click en el logo/header, abrir `https://procuradortool.com` en una pestaña nueva (`chrome.tabs.create`). Hoy no tiene ningún handler. (Hallazgo de R9.1/R9.2, probado a mano por el operador contra la extensión real v1.3.6.)
