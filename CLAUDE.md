...
| **SEC-2** | **Smoke tests CI + verificación diaria real** | — | ✅ **B.1 hecho (2026-07-13):** `docs/internal/sec2-b1-ci-setup.md`. Workflow `.github/workflows/smoke.yml` — `npm audit` (backend+electron, informativo) + smoke API (8 checks) + smoke de cobranza (`dev-tools/smoke-payments.js`, 19 checks) contra staging, por SSH con una **clave restringida dedicada** (forced-command, no la clave de administración general — no da shell, solo puede correr `ci-smoke.sh`). Verificado en vivo: 8/8 + 19/19, exit 0. ⏳ **B.2 pendiente** — verificación diaria real (procuración+informe reales con CUIT 27320694359, Windows Credential Manager, disparador al encender/manual/horario, panel de config, tarjeta en Diagnóstico + alerta >7 días). **Requiere release Electron** — plan en `plan-seguridad-precomercializacion-2026-07.md` (Parte B.2) |
